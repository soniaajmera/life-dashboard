...
const LifeDashboard = () => {
  ...
  // Save to storage
  useEffect(() => {
    const data = { habits, weeklyTasks, monthlyGoals, researchProjects, researchHistory, brainstormEntries, brainstormHistory, articleHistory, lastArticleDate, dailyArticle, lastWeeklyReset, lastMonthlyReset, lastHabitDate };
    localStorage.setItem('lifeDashboardData', JSON.stringify(data));
  }, [habits, weeklyTasks, monthlyGoals, researchProjects, researchHistory, brainstormEntries, brainstormHistory, articleHistory, lastArticleDate, dailyArticle, lastWeeklyReset, lastMonthlyReset, lastHabitDate]);
  ...
  useEffect(() => {
    const checkResets = () => {
      ...
      // Check daily habit reset (new day)
        ...
        setHabits(prev => {
          ...
          Object.keys(prev).forEach(key => {
            resetHabits[key] = {
              ...prev[key],
              today: false,
              streak: prev[key].today ? prev[key].streak : Math.max(0, prev[key].streak - 1)
            };
          });
          return resetHabits;
        });
      ...
      // Check weekly reset (Monday only)
